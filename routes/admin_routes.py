...
def admin_dashboard():
	...
	if selected_company_id:
		cursor.execute("SELECT id FROM companies WHERE id = %s", (selected_company_id,))
...
def add_company():
	if request.method == "POST":
		...
		if not company_name or not company_code or not email or not password:
			flash("Company name, code, email and password are required.", "error")
			return render_template("admin/add_company.html")

		connection = get_db_connection()
		cursor = connection.cursor(dictionary=True)
		
		# Check if company already exists in old companies table
		cursor.execute(
		...
		# Check if company already exists in new company table
		cursor.execute(
			"SELECT id FROM company WHERE company_code = %s OR email = %s",
			(company_code, email),
		)
		existing_new_company = cursor.fetchone()
		
		if existing_company or existing_new_company:
			cursor.close()
			connection.close()
			flash("Company code or email already exists.", "error")
			return render_template("admin/add_company.html")

		# Hash the password
		hashed_password = generate_password_hash(password)

		# Insert into old companies table for compatibility
		cursor.execute(
			"""
			INSERT INTO companies
			(company_name, company_code, address, email, attendance_module_enabled, canteen_module_enabled)
			VALUES (%s, %s, %s, %s, %s, %s)
			""",
			(company_name, company_code, address or None, email, attendance_module_enabled, canteen_module_enabled),
		)
		
		# Insert into new company table for login system
		cursor.execute(
			"""
			INSERT INTO company (name, email, password, company_code)
			VALUES (%s, %s, %s, %s)
			""",
			(company_name, email, hashed_password, company_code),
		)
		...
		flash("Company created successfully with login credentials.", "success")
		return redirect(url_for("admin.admin_dashboard"))
